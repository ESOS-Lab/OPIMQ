...
extern NVME_TASK_CONTEXT g_nvmeTask;
...
void handle_set_features(NVME_ADMIN_COMMAND *nvmeAdminCmd, NVME_COMPLETION *nvmeCPL)
{
	...
	switch(features.FID)
	{
		...
		case FID_INTERRUPT_COALESCING:
		{
			nvmeCPL->dword[0] = 0x0;
			nvmeCPL->specific = 0x0;
			break;
		}
		case FID_ARBITRATION:
		{
			nvmeCPL->dword[0] = 0x0;
			nvmeCPL->specific = 0x0;
			break;
		}
		case FID_ASYNCHRONOUS_EVENT_CONFIGURATION:
		{
			nvmeCPL->dword[0] = 0x0;
			nvmeCPL->specific = 0x0;
			break;
		}
		case FID_VOLATILE_WRITE_CACHE:
		{
...
void handle_get_features(NVME_ADMIN_COMMAND *nvmeAdminCmd, NVME_COMPLETION *nvmeCPL)
{
	...
	switch(features.FID)
	{
		...
		case FID_TEMPERATURE_THRESHOLD:
		{
			nvmeCPL->dword[0] = 0x0;
		...
		case FID_POWER_STATE_TRANSITION:
		{
			nvmeCPL->dword[0] = 0x0;
			nvmeCPL->specific = 0x0;
			break;
...
void handle_create_io_sq(NVME_ADMIN_COMMAND *nvmeAdminCmd, NVME_COMPLETION *nvmeCPL)
{
	...
	lld_nvme_set_io_sq(ioSqIdx, ioSqStatus->valid, ioSqStatus->cqVector, ioSqStatus->qSzie, ioSqStatus->pcieBaseAddrL, ioSqStatus->pcieBaseAddrH);
...
void handle_delete_io_sq(NVME_ADMIN_COMMAND *nvmeAdminCmd, NVME_COMPLETION *nvmeCPL)
{
	...
	lld_nvme_set_io_sq(ioSqIdx, 0, 0, 0, 0, 0);
...
void handle_create_io_cq(NVME_ADMIN_COMMAND *nvmeAdminCmd, NVME_COMPLETION *nvmeCPL)
{
	...
	lld_nvme_set_io_cq(ioCqIdx, ioCqStatus->valid, ioCqStatus->irqEn, ioCqStatus->irqVector, ioCqStatus->qSzie, ioCqStatus->pcieBaseAddrL, ioCqStatus->pcieBaseAddrH);
...
void handle_delete_io_cq(NVME_ADMIN_COMMAND *nvmeAdminCmd, NVME_COMPLETION *nvmeCPL)
{
	...
	lld_nvme_set_io_cq(ioCqIdx, 0, 0, 0, 0, 0, 0);
...
//	xil_printf("prpLen = %X, prp[1] = %X, prp[0] = %X\r\n",prpLen, prp[1], prp[0]);
	lld_hdma_set_direct_tx_dma(pIdentifyData, prp[1], prp[0], prpLen);
...
//		xil_printf("prpLen = %X, prp[1] = %X, prp[0] = %X\r\n",prpLen, prp[1], prp[0]);
	...
	lld_hdma_check_direct_tx_dma_done();
	nvmeCPL->dword[0] = 0;
	nvmeCPL->specific = 0x0;
}
...
void AdminCommand_process(NVME_COMMAND_ENTRY*p_cmdContext)
{
	NVME_COMPLETION nvmeCPL;
	NVME_ADMIN_COMMAND *p_adminCmd = (NVME_ADMIN_COMMAND*)p_cmdContext->cmdDword;

	unsigned int opc = (unsigned int)p_adminCmd->OPC;
	unsigned int needCpl = 1;
	unsigned int needSlotRelease = 0;

#if 0
	xil_printf("OPC = 0x%X\r\n", p_adminCmd->OPC);
	xil_printf("CID = 0x%X\r\n", p_adminCmd->CID);
	xil_printf("NSID = 0x%X\r\n", p_adminCmd->NSID);
	xil_printf("PRP1[63:32] = 0x%X, PRP1[31:0] = 0x%X\r\n", p_adminCmd->PRP1[1], p_adminCmd->PRP1[0]);
	xil_printf("PRP2[63:32] = 0x%X, PRP2[31:0] = 0x%X\r\n", p_adminCmd->PRP2[1], p_adminCmd->PRP2[0]);
	...
	switch(opc)
	{
		...
		{
			handle_set_features(p_adminCmd, &nvmeCPL);
			break;
		}
		case NVME_OPC_ADMIN_CREATE_IO_CQ:
		{
			handle_create_io_cq(p_adminCmd, &nvmeCPL);
			break;
		}
		case NVME_OPC_ADMIN_CREATE_IO_SQ:
		{
			handle_create_io_sq(p_adminCmd, &nvmeCPL);
			break;
		}
		case NVME_OPC_ADMIN_IDENTIFY:
		{
			handle_identify(p_adminCmd, &nvmeCPL);
			break;
		}
		case NVME_OPC_ADMIN_GET_FEATURES:
		{
			handle_get_features(p_adminCmd, &nvmeCPL);
			break;
		}
		case NVME_OPC_ADMIN_DELETE_IO_CQ:
		{
			handle_delete_io_cq(p_adminCmd, &nvmeCPL);
			break;
		}
		case NVME_OPC_ADMIN_DELETE_IO_SQ:
		{
			handle_delete_io_sq(p_adminCmd, &nvmeCPL);
			break;
		}
		case NVME_OPC_ADMIN_ASYNCHRONOUS_EVENT_REQUEST:
		{
			needCpl = 0;
			needSlotRelease = 1;
			nvmeCPL.dword[0] = 0;
		...
		{
			handle_get_log_page(p_adminCmd, &nvmeCPL);
			break;
		}
		case NVME_OPC_ADMIN_SECURITY_RECEIVE:
		{
			needCpl = 0;
			needSlotRelease = 0;
			nvmeCPL.dword[0] = 0;
		...
		case NVME_OPC_ADMIN_DOORBELL_BUFFER_CONFIG:
		{
			needCpl = 0;
			needSlotRelease = 0;
			nvmeCPL.dword[0] = 0;
		...
		case NVME_OPC_ADMIN_ABORT:
		{
			// SP: temp, success completion
			needCpl = 1;
			needSlotRelease = 0;
			nvmeCPL.dword[0] = 0;
			nvmeCPL.specific = 0x0;
		...
		default:
		{
			...
			nvmeCPL.dword[0] = 0;
			nvmeCPL.statusFieldWord = SC_INVALID_FIELD_IN_COMMAND;
			break;
		}
	}
	...
	if(needCpl == 1)
	{
		hal_host_completion_nvme_command(p_cmdContext->cmdSlotTag, nvmeCPL.specific, nvmeCPL.statusFieldWord);
	}
	else if(needSlotRelease == 1)
	{
		lld_nvme_set_slot_release(p_cmdContext->cmdSlotTag);
	}
	else
	{
	...
	//set_nvme_cpl(p_cmdContext->qID, p_adminCmd->CID, nvmeCPL.specific, nvmeCPL.statusFieldWord);
