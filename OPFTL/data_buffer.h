...
#define DATA_BUF_IDX_INVALID	0xffff
...
typedef struct _DATA_BUF_ENTRY {
	...
	unsigned int mappable_1;
	unsigned int mappable_2;
...
//SP: new buffer management
typedef struct {
	unsigned int lsa;
	unsigned int lba_bitmap;
} buf_entry_t;

typedef struct {
	unsigned int bitmap[AVAILABLE_TEMPORARY_DATA_BUFFER_ENTRY_COUNT/32]; // bit set: free, bit clear: in use
...
void BufferManagement_UpdateBufEntryInfoBlockingReq(unsigned int bufEntry, unsigned int reqSlotTag);
...
void BufferManagement_AddBufToHashList(unsigned int bufEntry);
