...
./QUICK_TEST/ : Containing quick test script for performing the getting started instructions        ./scripts/ : The script path for performing the experiments in the Detailed Instructions.
         * CONTAINER_THROGHPUT/ 
         * EPOCH_PIN/ 
         * FSYNC_LATENCY/ 
...
We have precompiled the kernels required for the experiments, and you can boot into the desired kernel by executing the following script. To select and boot a kernel based on the experimental environment, run the following script:
./kernel.sh
Please enter the kernel name to execute: 
   B - Baseline 
   O - OPIMQ 
   M - MQFS 
   BFS - BFS with SQ
   N   - OPIMQ without epoch pinning
 // Type the corresponding string and press Enter
sudo reboot
	The boot process takes approximately 2–3 minutes.
The names and descriptions of the kernels to be used in the experiments are as follows:
         * 5.18.18+original: Vanilla kernel without OPIMQ (Baseline).
         * 5.18.18-opext4+opimq: Kernel with OPIMQ enabled (Proposed Solution).
         * 4.18.20-MQFS:  Kernel that includes ccNVMe and MQFS, used for comparison with OPIMQ.
         * 5.18.18-bfs+sq: The kernel that can only use a single command queue in BarrierFS.
