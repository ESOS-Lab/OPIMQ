...
int ext4_sync_file(struct file *file, loff_t start, loff_t end, int datasync)
{
	...
	//	,__func__,task_pid(current),commit_tid); 
out:
//	filemap_fdatawait_range(file->f_mapping, start, end); // Jieun add
//	err = file_check_and_advance_wb_err(file);
	if (ret == 0)
		ret = err;
#ifdef OPIMQ_DEBUG
	end2 = ktime_get();
	trace_ext4_sync_file_exit(inode, ret, start1, end1, end2);
#endif
	return ret;
}
