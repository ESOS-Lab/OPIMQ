...
#ifdef ENABLE_OPIMQ
static struct request *__blk_mq_alloc_requests_opimq(struct blk_mq_alloc_data *data)
{
	struct request_queue *q = data->q;
	u64 alloc_time_ns = 0;
	struct request *rq;
...
//	next_cpu = blk_mq_hctx_next_cpu(hctx);

//	if (!strcmp(comm, "fio")) {
//		printk(KERN_ERR "[BFS-DEBUG] PID:%d CPU:%d Next CPU:%d\n",
//			current->pid, raw_smp_processor_id(), next_cpu);
//		dump_stack();
//	}		
//	kblockd_mod_delayed_work_on(next_cpu, &hctx->run_work,
//				    msecs_to_jiffies(msecs));
					       ...
					       unsigned int nsegs)
{
	...
	if (bio->ordered_flag > 0) {
		rq = __blk_mq_alloc_requests_opimq(&data);
	} else {
		rq = __blk_mq_alloc_requests(&data);
	}
#else
