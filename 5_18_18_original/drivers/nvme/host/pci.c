	...
	for (i = 0, qoff = 0; i < set->nr_maps; i++) {
		...
		/*
			...
			if (dev->ctrl.instance == 0) {
				blk_mq_pci_map_queues_debug(map, to_pci_dev(dev->dev), offset);			
			} else {
						...
						struct nvme_queue *nvmeq)
{
	...
	// JIEUN Debugging
	printk(KERN_ERR "[JIEUN-DEBUG] NVMe devnum%d\t qid %d qsize %d", dev->ctrl.instance, qid, nvmeq->q_depth-1);
...
static int nvme_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	//Debugging
	printk(KERN_ERR "[JIEUN-DEBUG] NVMe dev numa node %d\t devnum%d nr_allocated_queues %d", node, dev->ctrl.instance, dev->nr_allocated_queues);
