...
/*
	    ...
	    TP_printk("PID %d cpuid %d qid %d cmdid %u cmd=(%s %s)",
...
TRACE_EVENT(nvme_complete_rq,
		...
		*/
	    TP_printk("PID %d cpuid %d qid %d cmdid=%u, res=%#llx, retries=%u, flags=0x%x, status=%#x",
		      __entry->pid, __entry->cpuid,
		      __entry->qid, __entry->cid, __entry->result,
		      __entry->retries, __entry->flags, __entry->status)
);
...
*/
	TP_printk("PID %d qid %d cpuid %d",
		__entry->pid, __entry->qid, __entry->cpuid
	)
);
...
TRACE_EVENT(nvme_sq,
	...
	TP_fast_assign(
		...
		__entry->iolat = complete - req->start; // DMA transfer time
		__entry->cid = nvme_req(req)->cmd->common.command_id;
		__entry->cpuid = raw_smp_processor_id();
		__entry->pid = current->pid;
	),
