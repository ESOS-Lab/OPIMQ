...
*/
	...
	TP_printk("dev %d ino %lu parent %lu datasync %d PID %d cpuid %d",
		  MAJOR(__entry->dev),
		  (unsigned long) __entry->ino,
		  (unsigned long) __entry->parent, __entry->datasync, __entry->pid, __entry->cpuid)
);

TRACE_EVENT(ext4_sync_file_exit,
	TP_PROTO(struct inode *inode, int ret, s64 start, s64 end1, s64 end2),

	TP_ARGS(inode, ret, start, end1, end2),
...
*/
	...
	TP_printk("PID %d cpuid %d ino %lu total_time %llu data %llu journal %llu",
		  __entry->pid, __entry->cpuid,
		  (unsigned long) __entry->ino,
		  __entry->total, __entry->data_write, __entry->journal_write)
);
