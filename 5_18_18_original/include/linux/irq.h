...
static inline struct cpumask *irq_data_get_affinity_mask(struct irq_data *d)
{
	//Jieun add
/*
	if (d->irq == 97 || d->irq == 94){
		//cpumask_setall(d->common->effective_affinity);
		printk(KERN_ERR "[JIEUN DEBUG] Before irq_data_get_affinity_mask irq %d effective_affinity %*pbl", d->irq, cpumask_pr_args(d->common->affinity));	
		//cpumask_copy(d->common->effective_affinity, d->common->affinity);	
		cpumask_setall(d->common->affinity);
		printk(KERN_ERR "[JIEUN DEBUG] After irq_data_get_affinity_mask irq %d effective_affinity %*pbl", d->irq, cpumask_pr_args(d->common->affinity));	
	} */
...
struct cpumask *irq_data_get_effective_affinity_mask(struct irq_data *d)
{
	...
	if (d->irq == 97 || d->irq == 94){
		//cpumask_setall(d->common->effective_affinity);
		printk(KERN_ERR "[JIEUN DEBUG] get_effective_affinity irq %d effective_affinity %*pbl", d->irq, cpumask_pr_args(d->common->effective_affinity));	
		//cpumask_copy(d->common->effective_affinity, d->common->affinity);
	} 
	*/
						      ...
						      const struct cpumask *m)
{
	...
	if (d->irq == 97 || d->irq == 94){
		printk(KERN_ERR "[JIEUN DEBUG] update_effective_affinity irq %d mask %*pbl", d->irq, cpumask_pr_args(m));	
		//dump_stack();
		//cpumask_set_cpu(5, d->common->effective_affinity);
...
//Jieun add
int irq_matrix_alloc_managed_target_cpu(struct irq_matrix *m, const struct cpumask *msk,
				unsigned int *mapped_cpu);
