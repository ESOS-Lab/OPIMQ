...
/*
	TP_printk("dev %d,%d ino %lu PID %d parent %lu datasync %d ",
		  MAJOR(__entry->dev), MINOR(__entry->dev),
		  (unsigned long) __entry->ino, __entry->pid,
		  (unsigned long) __entry->parent, __entry->datasync)
*/
	TP_printk("PID %d cpuid %d datasync %d ",
		  __entry->pid, __entry->cpuid,  __entry->datasync)
);

TRACE_EVENT(ext4_fbarrier_file_enter,
	TP_PROTO(struct file *file, int datasync),
...
TRACE_EVENT(ext4_sync_file_data_start,
...
TRACE_EVENT(ext4_sync_file_data_complete,
	TP_PROTO(struct inode *inode, int ret),
...
TRACE_EVENT(ext4_sync_file_exit,
	TP_PROTO(struct inode *inode, int ret, s64 start, s64 end1, s64 end2, int datasync, int needs_barrier),

	TP_ARGS(inode, ret, start, end1, end2, datasync, needs_barrier),
...
//		__field(	dev_t,	dev			)
		...
		__field(	int,	ret			)
		__field(	s64,	total		)
		__field(	s64,	data_write	)
		__field(	s64,	journal_write)
		__field(	int,	pid)
		__field(	int,	cpuid)
		__field(	int,	datasync)
		__field(	int,	needs_barrier)
	),
...
//		__entry->dev		= inode->i_sb->s_dev;
		__entry->ino		= inode->i_ino;
		__entry->ret		= ret;
		__entry->total		= end2-start;
		__entry->data_write		= end1-start;
		__entry->journal_write		= end2-end1;
		__entry->pid	= current->pid;
		__entry->datasync = datasync;
		__entry->needs_barrier = needs_barrier;
		__entry->cpuid = raw_smp_processor_id();
	),
/*
	TP_printk("PID %d ino %lu total_time %llu data %llu journal %llu datasync %d needs_barrier %d ",
		  __entry->pid, 
		  (unsigned long) __entry->ino,
		  __entry->total, __entry->data_write, __entry->journal_write, __entry->datasync, __entry->needs_barrier)
*/
	TP_printk("PID %d cpuid %d total_time %llu data %llu journal %llu datasync %d needs_barrier %d ",
		  __entry->pid, __entry->cpuid,
		  __entry->total, __entry->data_write, __entry->journal_write, __entry->datasync, __entry->needs_barrier)
);

TRACE_EVENT(ext4_fbarrier_file_exit,
	TP_PROTO(struct inode *inode,  s64 start, s64 end1, s64 end2, int datasync),

	TP_ARGS(inode, start, end1, end2, datasync),
...
//		__entry->ret		= ret;
	...
	TP_printk("PID %d total_time %llu data %llu journal %llu datasync %d",
		  __entry->pid, __entry->total, __entry->data_write, __entry->journal_write, __entry->datasync)
