...
#ifdef ENABLE_OPIMQ
	bio->ordered_flag = bio_src->ordered_flag;
	bio->j_task = bio_src->j_task;
	bio->tx_id = bio_src->tx_id;
#endif
		      ...
		      gfp_t gfp, struct bio_set *bs)
{
	...
	// Jieun add
	//printk(KERN_ERR "[bio_split] PID %d bio->bi_size %d split->bi_size %d\n",
	//	current->pid, bio->bi_iter.bi_size, split->bi_iter.bi_size);	
	//opimq
	if ((current->pid > 2000) && (bio_op(bio) == REQ_OP_WRITE) && (bio->ordered_flag >0 && split->ordered_flag ==0)) {
		printk(KERN_ERR "[bio_split] PID: %d bio->ordered_flag %d split->ordered_flag %d\n",
			current->pid, bio->ordered_flag, split->ordered_flag);	
		dump_stack();
	}
