...
//	if (current->devname == "nvme0q1") {
//		printk(KERN_ERR "[JIEUN DEBUG] Before assign_managed_vector irq %d name %s cpu %d", irqd->irq, irqd->chip->name, cpu);
//		vector = irq_matrix_alloc_managed_target_cpu(vector_matrix, vector_searchmask,
//						  &cpu);

//		current->devname = NULL;
//	} else {
//		vector = irq_matrix_alloc_managed(vector_matrix, vector_searchmask,
//						  &cpu);
//	}
	...
	// Jieun add for debugging
//	if (strcmp(current->devname, "nvme0q")) {
//		printk(KERN_ERR "[JIEUN DEBUG] After assign_managed_vector irq %d cpu %d", irqd->irq, cpu);
//	}
	apic_update_vector(irqd, vector, cpu);
	apic_update_irq_cfg(irqd, vector, cpu);
	return 0;
