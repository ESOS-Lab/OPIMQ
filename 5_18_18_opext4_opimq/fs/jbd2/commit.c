...
#ifdef ENABLE_OPIMQ
	if (commit_transaction != NULL) {
		tid = commit_transaction->t_tid;
	} else {
		tid = 0;
	}
#endif
...
#ifdef ENABLE_OPIMQ	
		ret = submit_bh_opimq(REQ_OP_WRITE,
			REQ_SYNC, bh, 1, 1, tid);
#else
...
#ifdef ENABLE_OPIMQ
		ret = submit_bh_opimq(REQ_OP_WRITE, REQ_SYNC, bh, 1, 1, tid);
#else
...
//#ifdef ENABLE_OPIMQ
//	wake_up(&journal->j_wait_dispatch_commit);	//fbarrier() or fdatabarrier().
//#endif
	...
	while (commit_transaction->t_buffers) {
		...
		/*
		if (bufs < 70)
			orig_blocknrs[bufs] = jh2bh(jh)->b_blocknr;
		*/
...
start_journal_io:
			...
			for (i = 0; i < bufs; i++) {
				...
				if (i == bufs - 1) 
					submit_bh_opimq(REQ_OP_WRITE, REQ_SYNC, bh, 1, 0, tid);
				else
					submit_bh_opimq(REQ_OP_WRITE, REQ_SYNC, bh, 0, 0, tid);
#else
