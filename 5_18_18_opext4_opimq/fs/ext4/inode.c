...
#ifdef ENABLE_OPIMQ
	if (mpd->wbc->sync_mode == WB_SYNC_ALL || mpd->wbc->sync_mode == WB_ORDERED_ALL || mpd->wbc->tagged_writepages)
...
#ifdef ENABLE_OPIMQ
	if (wbc->sync_mode == WB_SYNC_ALL || wbc->tagged_writepages || wbc->sync_mode == WB_ORDERED_ALL)
...
#ifdef ENABLE_OPIMQ
	if (mpd.wbc->sync_mode == WB_ORDERED_ALL && sbi->s_journal && sbi->s_journal->j_task) {
		if (current->pid != sbi->s_journal->j_task->pid) {	//current JBD thread
			mpd.io_submit.j_task = sbi->s_journal->j_task;
		} else {
			mpd.io_submit.j_task = NULL;
		} 
			//mpd.io_submit.tx_id = 0;
		if (sbi->s_journal->j_running_transaction) {
			mpd.io_submit.tx_id = sbi->s_journal->j_running_transaction->t_tid;
		//	printk(KERN_ERR "[OPIMQ-DEBUG] ext4_writepages [1] PID: %d TXID: %d\n", 
		//			current->pid, mpd.io_submit.tx_id);
		} 
	}
...
#ifdef ENABLE_OPIMQ
	if (&mpd.io_submit.io_bio && mpd.wbc->sync_mode == WB_ORDERED_ALL) {
		mpd.io_submit.barrier_flag = 1;
		//current->is_barrier = 1;
	}

	mpd.io_submit.barrier_flag = 0;
#endif
...
#ifdef ENABLE_OPIMQ
		if (mpd.wbc->sync_mode == WB_ORDERED_ALL && handle != NULL) {
			//printk(KERN_ERR "[OPIMQ-DEBUG] ext4_writepages [2] PID: %d\n", 
			//		current->pid);

			if (current->pid != handle->h_transaction->t_journal->j_task->pid) {	
				mpd.io_submit.j_task = handle->h_transaction->t_journal->j_task;	// App thread
			} else {
				mpd.io_submit.j_task = NULL;	// JBD thread
			}

			if (handle->h_transaction) {
				mpd.io_submit.tx_id = handle->h_transaction->t_tid;
			//	printk(KERN_ERR "[OPIMQ-DEBUG] ext4_writepages [3] PID: %d TXID: %d\n", 
			//		current->pid, mpd.io_submit.tx_id);
			}
		}
#endif
