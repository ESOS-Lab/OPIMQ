			 ...
			 const struct blk_mq_queue_data *bd)
{
	...
	if (unlikely(ret))
		return ret;
	start = ktime_get();	//Debugging
	req->start = start;	//Debugging
	spin_lock(&nvmeq->sq_lock);
	nvme_sq_copy_cmd(nvmeq, &iod->cmd);
	nvme_write_sq_db(nvmeq, bd->last);
	blk_mq_dispatch_request(req);
	spin_unlock(&nvmeq->sq_lock);
	req->start2 = ktime_get();
	return BLK_STS_OK;
}

static void nvme_submit_cmds(struct nvme_queue *nvmeq, struct request **rqlist)
{
	ktime_t	start;	//Debugging
	start = ktime_get();	//Debugging
	struct request *req;
	spin_lock(&nvmeq->sq_lock);
...
#ifdef ENABLE_OPIMQ
static void nvme_queue_rqs(struct request **rqlist, bool plug_finish)
#else
...
#endif
{
	...
	rq_list_for_each_safe(rqlist, req, next) {
		...
		trace_nvme_getrq(req);
#ifdef ENABLE_OPIMQ
		// Assign epoch id & update epoch counter
		if (req->stream_id_1 > 0) {

//			if (!(strcmp(comm, "filebench_share"))){
//				printk(KERN_ERR "[OPIMQ] nvme_queue_rqs dispatch! pid comm: PID %d, comm: %s\n", current->pid, comm);
//			}
			...
			if (plug_finish) {
				if (!next && current->is_barrier) {	//last dispatched write request.
					req->barrier_flag = 1;
					current->is_barrier = 0;
					current->queue_id = -1;
					req->epoch_id_1 = atomic_fetch_add(1, &current->epoch_counter);
...
#endif
		/*	
		if ((current->pid> 1500) && (req->stream_id_1 == 0)) {
			//dump_stack();
			printk(KERN_ERR "[Request Create] PID %d plug_finish %d sid1 %d eid1 %d sid2 %d eid2 %d bflag %d\n",
					current->pid, plug_finish, req->stream_id_1, req->epoch_id_1, req->stream_id_2, req->epoch_id_2, req->barrier_flag); 
		}*/


		trace_nvme_setrq(req);
				   ...
				   struct io_comp_batch *iob, u16 idx)
{
	...
	/* Debugging */
	complete = ktime_get();
	trace_nvme_sq(req, cqe->sq_head, nvmeq->sq_tail, complete);
	req->start = 0;
...
//	dev->nr_allocated_queues = 2;
	
//#ifdef QUEUE_TEST_OPIMQ
//	dev->nr_allocated_queues = 41;
//#endif
