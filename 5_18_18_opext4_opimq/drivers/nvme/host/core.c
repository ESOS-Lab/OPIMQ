		...
		enum nvme_opcode op)
{
	...
	//	printk(KERN_ERR "[nvme_setup_rw] sid1 %d eid1 %d sid2 %d eid2 %d bflag %d\n",
	//		req->stream_id_1, req->epoch_id_1, req->stream_id_2, req->epoch_id_2, req->barrier_flag);

	//}


#ifdef ENABLE_OPIMQ
	cmnd->rw.stream_id_1 = req->stream_id_1;
	cmnd->rw.stream_id_2 = req->stream_id_2;
	cmnd->rw.epoch_id_1 = req->epoch_id_1;
	cmnd->rw.epoch_id_2 = req->epoch_id_2;
#else
	cmnd->rw.cdw2 = 0;
	cmnd->rw.cdw3 = 0;
#endif
...
int nvme_set_queue_count(struct nvme_ctrl *ctrl, int *count)
{
	...
	//Jieun Add
	printk(KERN_ERR "[OPIMQ] dev_id: %d\t nvme_set_queue_count1 q_count: %d\n", ctrl->instance, *count);
	...
	if (status > 0) {
		...
		*count = 0;
		...
		printk(KERN_ERR "[OPIMQ] dev_id: %d\t status: %d\t nvme_set_queue_count2 result: %d\n", ctrl->instance, status, result >> 16);
		nr_io_queues = min(result & 0xffff, result >> 16) + 1;
		printk(KERN_ERR "[OPIMQ] dev_id: %d\t status: %d\t nvme_set_queue_count3 nr_io_queues: %d\n", ctrl->instance, status, nr_io_queues);
		*count = min(*count, nr_io_queues);
		printk(KERN_ERR "[OPIMQ] dev_id: %d\t status: %d\t nvme_set_queue_count4 count: %d\n", ctrl->instance, status, *count);
	}
